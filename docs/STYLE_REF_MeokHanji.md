# Meok & Hanji (먹과 한지) — Style Reference
...
캔버스는 민무늬 단색이 아니라 **실제 한지의 물성**을 갖는다. 구현은 사용자 제공 한지 스캔(닥나무 섬유 결 실물, 원본 [[hanji-texture.avif]] 493×740)을 **2×2 거울 타일**로 가공한 [[hanji-texture-tile.webp]] (986×1480, 22KB — 좌우·상하 반전 조합으로 반복 이음매 제거):

```css
background-color: var(--hanji);
background-image: url('hanji-texture-tile.webp');
